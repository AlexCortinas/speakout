<div class="wrap" id="dk-speakout">
	...
	<h2><?php _e( 'Signatures', 'speakout' ); ?></h2>
				...
				<form action="" method="get">
					...
					<?php
						...
						if( isset( $_REQUEST['pid'] ) || $pid != '' ) {
							echo ' 
								<a class="button dk-speakout-inline" style="margin: 0 .5em 0 .5em" href="' .  esc_url( wp_nonce_url( $csv_url . '&csv=signatures', 'dk_speakout-download_signatures' ) ) . '">' . __( 'Download as CSV', 'speakout' ) . '</a>
								<a id="dk-speakout-reconfirm" class="button dk-speakout-inline" href="' . esc_url( wp_nonce_url( $reconfirm_url, 'dk_speakout-resend_confirmations' . $pid ) ) . '">' . __( 'Re-send confirmations', 'speakout' ) . '</a>
	...
	<table class="widefat">
		<thead>
			<tr>
				<th></th>
				<th><?php _e( 'Name', 'speakout' ); ?></th>
				<th><?php _e( 'Email', 'speakout' ); ?></th>
				<th><?php _e( 'Petition', 'speakout' ); ?></th>
				<th><?php _e( 'Confirmed', 'speakout' ); ?></th>
				<th><?php _e( 'Opt-in', 'speakout' ); ?></th>
				<th><?php _e( 'Date', 'speakout' ); ?></th>
				<th></th>
			</tr>
		</thead>
		<tfoot>
		...
		<tbody>
			...
			<?php foreach ( $signatures as $signature ) : ?>
				...
				<?php
					...
					if ( $confirmed == '1' )
						$confirmed = '<span class="dk-speakout-green">'  . __( 'confirmed', 'speakout' ) . '</span>';
					elseif ( $confirmed == '0' )
						$confirmed = __( 'unconfirmed', 'speakout' );
					else
						$confirmed = '...';
					// make email opt-in values readable
					$optin = $signature->optin;
					if ( $optin == '1' )
						$optin = '<span class="dk-speakout-green">'  . __( 'yes', 'speakout' ) . '</span>';
					elseif ( $optin == '0' )
			...
			<tr class="dk-speakout-tablerow">
				...
				<td><?php echo stripslashes( esc_html( $signature->honorific . ' ' . $signature->first_name . ' ' . $signature->last_name ) ); ?></td>
				...
				<td class="dk-speakout-right"><span class="trash"><a href="<?php echo $delete_url; ?>"><?php _e( 'Delete', 'speakout' ); ?></a></span></td>
			</tr>
			<?php endforeach; ?>
		</tbody>
	</table>
