...
function dk_speakout_install() {
	...
	$sql_create_tables = "
		CREATE TABLE `$db_petitions` (
			...
			`custom_field_label` VARCHAR(200) CHARACTER SET utf8 NOT NULL DEFAULT 'Custom field',
			`displays_custom_message` CHAR(1) BINARY NOT NULL,
			`custom_message_label` VARCHAR(200) CHARACTER SET utf8 NOT NULL,
		...
		CREATE TABLE `$db_signatures` (
			...
			`honorific` VARCHAR(8) CHARACTER SET utf8 NOT NULL,
			...
			`is_confirmed` CHAR(1) BINARY NOT NULL DEFAULT '0',
	...
	$options = array(
		...
		"confirm_message"        => __( "Hello", "dk_speakout" ) . " %first_name%\r\n\r\n" . __( "Thank you for signing our petition %petition_title% ", "dk_speakout" ). ". " . __( "Please confirm your email address by clicking or copying and pasting the link below:", "dk_speakout" ) . "\r\n%confirmation_link%\r\n\r\n" . get_bloginfo( "name" ),
...
$sql_update = "
			ALTER TABLE $db_signatures ADD `honorific` VARCHAR(8) NULL AFTER `petitions_id`
		;";
	...
	if ( $installed_version != $dk_speakout_version ) { 
		...
		ADD `displays_custom_message` CHAR(1) NOT NULL AFTER `custom_field_label`, 
		ADD `custom_message_label` VARCHAR(200) NOT NULL AFTER `displays_custom_message`
		;";
		$wpdb->query( $sql_update );
		...
		if ( ! array_key_exists( 'confirm_message', $options ) ) {
			$options['confirm_message'] = __( "Hello", "dk_speakout" ) . " %first_name%\r\n\r\n" . __( "Thank you for signing our petition", "dk_speakout" ) . ". " . __( "Please confirm your email address by clicking or copying and pasting the link below:", "dk_speakout" ) . "\r\n%confirmation_link%\r\n\r\n" . get_bloginfo( "name" );
		}
