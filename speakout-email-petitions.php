...
Author URI: https://123host.com.au
License: GPL2
*/
...
$dk_speakout_version = '1.6.2' ;

// enable localizations
add_action( 'init', 'dk_speakout_translate' );
function dk_speakout_translate() {
	load_plugin_textdomain( 'speakout', false, 'speakout/languages' );
}
...
function dk_speakout_meta_links( $links, $file ) {
	...
	if ( $file == $plugin ) {
		return array_merge(
			$links,
			array(
				sprintf( '<a href="http://wordpress.org/tags/speakout-email-petitions?forum_id=10">%s</a>', __( 'Support', 'speakout' ) ),
				sprintf( '<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=4PPYZ8K2KLXUJ">%s</a>', __( 'Donate', 'speakout' ) )
			)
		);
	}
